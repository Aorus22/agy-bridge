...
export function Transcript({ run }: { run: Run | null }) {
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0 overflow-hidden bg-background">
      {/* Minimal Header: Conv ID (mono) + cwd (truncated) + elapsed */}
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-border text-xs min-h-[41px]">
        ...
        <div className="flex items-center gap-3 text-muted-foreground/60 text-xs font-mono shrink-0 ml-4">
          {run.cwd && (
      ...
      {/* Result Footer: Only status has a badge, clean tokens & final response */}
      {run.result && (
        <div className="px-4 py-3 border-t border-border bg-card/20 space-y-2.5">
          ...
          {run.result.usage && (
            <div className="flex flex-wrap gap-x-3 gap-y-1 font-mono text-[10px] text-muted-foreground/50">
          ...
          {run.result.error && (
            <div className="font-mono text-xs text-red-400 bg-red-950/20 border border-red-900/30 rounded px-2.5 py-1.5 whitespace-pre-wrap break-words">
              {run.result.error}
            </div>
          )}
          ...
          {run.result.response && run.result.response.trim() && (
            <>
              ...
              <div className="text-xs bg-muted/20 border border-border/50 rounded-md p-3 max-h-48 overflow-y-auto whitespace-pre-wrap break-words leading-relaxed text-foreground/90 font-sans">
